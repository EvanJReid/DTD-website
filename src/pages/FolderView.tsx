...
export default function FolderView() {
  ...
  const handleDownloadAll = () => {
    ...
    a.href = `${BASE_URL}/folders/${folderId}/download-zip`;
    a.click();
  };
      ...
      <div className="border-b border-border bg-card/50 backdrop-blur-sm sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-6 py-4">
          ...
          <div className="flex items-start justify-between gap-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap items-center gap-4 mt-3 text-sm text-muted-foreground">
                  ...
                  <span className="inline-flex items-center gap-1.5">
                    <User className="h-4 w-4" />{folder.professor}
                  </span>
                  <span className="inline-flex items-center gap-1.5">
                    <FileText className="h-4 w-4" />{folder.documentCount} documents
                  </span>
                  <span className="inline-flex items-center gap-1.5">
                    <Calendar className="h-4 w-4" />Created {formatDate(folder.createdAt)}
                  </span>
                </div>
              </div>
            </div>
            ...
            {documents.length > 0 && (
              <Button onClick={handleDownloadAll} variant="outline" className="shrink-0">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-lg font-semibold">
            {documents.length} file{documents.length !== 1 ? 's' : ''} in this collection
          </h2>
          {documents.length > 0 && (
