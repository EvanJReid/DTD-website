...
import { ArrowLeft, Plus, Building2, Users, Search, Briefcase, ChevronDown } from "lucide-react";
...
const Coops = () => {
  const { coopsByCompany, coops, addCoop } = useCoops();
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [searchQuery, setSearchQuery] = useState("");
  const [expandedCompanies, setExpandedCompanies] = useState<Set<string>>(new Set());

  const handleAddCoop = (data: CoopFormData) => {
    addCoop(data);
  ...
  const currentCoops = coops.filter((c) => c.status === "current").length;
          ...
          <div className="flex h-16 items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Where brothers have worked</p>
              </div>
            </div>
            <Button onClick={() => setIsModalOpen(true)} size="sm">
              <Plus className="h-4 w-4" />Add Co-op
            </Button>
          </div>
        </div>
      </header>
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            {!searchQuery && (
              <Button onClick={() => setIsModalOpen(true)} className="mt-4">
                <Plus className="h-4 w-4 mr-2" />Add Co-op
              </Button>
            )}
          </div>
        ) : (
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            ...
                            {currentCount > 0 && (
                              <span className="text-green-600"> · {currentCount} current</span>
                            )}
                          </p>
                        </div>
                      </div>
